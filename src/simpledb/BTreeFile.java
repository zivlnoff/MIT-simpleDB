...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.INTERNAL) {
            ...
            while (iterator.hasNext()) {
                next = iterator.next();
                if (f == null || f.compare(Op.LESS_THAN, next.getKey())) {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        BTreePageId bTreePageId = new BTreePageId(tableid, getEmptyPageNo(tid, dirtypages), BTreePageId.LEAF);
        BTreeLeafPage newLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, bTreePageId, Permissions.READ_WRITE);
        Iterator<Tuple> tupleIterator = page.reverseIterator();
        int count = page.getNumTuples() / 2;
        Tuple copyUpTuple = page.getTuple(count);
        Field copyUpField = copyUpTuple.getField(keyField);
        ...
        page.setRightSiblingId(bTreePageId);
        if (rightSiblingId != null) {
            BTreeLeafPage rightLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
            rightLeafPage.setLeftSiblingId(bTreePageId);
        }
        // Copy the middle key up into the parent page, and recursively split the parent as needed to accommodate
        // the new entry.  getParentWithEmtpySlots() will be useful here.

        BTreeEntry copyUpEntry = new BTreeEntry(copyUpField, page.getId(), bTreePageId);

        BTreeInternalPage parentWithEmptySlots = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), copyUpField);

        parentWithEmptySlots.insertEntry(copyUpEntry);
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Field copyUpField = page.getKey(count);
        while (count > 0 && bTreeEntryIterator.hasNext()) {
        ...
        if (bTreeEntryIterator.hasNext()) {
            page.deleteKeyAndRightChild(bTreeEntryIterator.next());
        }
        // Push the middle key up into the parent page, and recursively split the parent as needed to accommodate
        // the new entry.  getParentWithEmptySlots() will be useful here.
        BTreeEntry newEntry = new BTreeEntry(copyUpField, page.getId(), newInternalPage.getId());
        BTreeInternalPage parentWithEmptySlots = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), copyUpField);
        ...
        page.setParentId(parentWithEmptySlots.getId());
        newInternalPage.setParentId(parentWithEmptySlots.getId());
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int perPageNum = (page.getNumTuples() + sibling.getNumTuples()) / 2;
        int transNum = perPageNum - page.getNumTuples();
        Tuple next = null;
        if (isRightSibling == false) {
            Iterator<Tuple> tupleIterator = sibling.reverseIterator();
            while (transNum > 0 && tupleIterator.hasNext()) {
            ...
            assert (next != null);
            Field key = next.getField(keyField);
            entry.setKey(key);
        } else {
            Iterator<Tuple> iterator = sibling.iterator();
            while (transNum > 0 && iterator.hasNext()) {
                next = iterator.next();
                sibling.deleteTuple(next);
                page.insertTuple(next);
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int perPageNum = (page.getNumEntries() + leftSibling.getNumEntries()) / 2;
        int transNum = perPageNum - page.getNumEntries();
        BTreeEntry next = null;
        parent.deleteKeyAndRightChild(parentEntry);
        while (transNum > 0 && bTreeEntryIterator.hasNext()) {
            next = bTreeEntryIterator.next();
            leftSibling.deleteKeyAndRightChild(next);
            if (transNum == perPageNum - page.getNumEntries()) {
                parentEntry.setLeftChild(next.getRightChild());
                Iterator<BTreeEntry> iterator = page.iterator();
                if (iterator.hasNext()) {
                    BTreeEntry firstEntry = iterator.next();
                    parentEntry.setRightChild(firstEntry.getLeftChild());
                }
                page.insertEntry(parentEntry);
            }
            if (transNum == 1) {
                next.setLeftChild(leftSibling.getId());
                next.setRightChild(page.getId());
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int perPageNum = (page.getNumEntries() + rightSibling.getNumEntries()) / 2;
        int transNum = perPageNum - page.getNumEntries();
        BTreeEntry next = null;
        parent.deleteKeyAndRightChild(parentEntry);
        while (transNum > 0 && bTreeEntryIterator.hasNext()) {
            ...
            if (transNum == perPageNum - page.getNumEntries()) {
                ...
                if (iterator.hasNext()) {
                    BTreeEntry lastEntry = iterator.next();
                    parentEntry.setLeftChild(lastEntry.getRightChild());
                }
                page.insertEntry(parentEntry);
            }
            if (transNum == 1) {
                next.setLeftChild(page.getId());
                next.setRightChild(rightSibling.getId());
                parent.insertEntry(next);
                updateParentPointers(tid, dirtypages, page);
                transNum--;
                continue;
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        parentEntry.setRightChild(iterator.next().getLeftChild());
